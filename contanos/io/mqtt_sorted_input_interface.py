...
class MQTTSortedInput(ABC):
    ...
    def _extract_frame_id(self, payload_data: Any) -> Optional[int]:
        ...
        frame_id = payload_data.get('frame_id')
        if not frame_id or not isinstance(frame_id, str):
            return None

        try:
            # Extract frame ID using split('FRAME:')[-1]
            frame_id_part = frame_id.split('FRAME:')[-1]
            return int(frame_id_part)
        except (ValueError, IndexError):
            logging.warning(f"Failed to extract frame ID from: {frame_id}")
            return None
    ...
    async def read_data(self) -> Tuple[Any, Dict[str, Any]]:
        ...
        try:
            ...
            metadata = {
                'frame_id': payload_data.get('frame_id') if isinstance(payload_data, dict) else None,
                'frame_id': frame_id,
                'topic': message['topic'],
                'qos': message['qos'],
