...
class RTSPOutputManager:
    ...
    def _stream_worker(self, task_id: str):
        ...
        try:
            ...
            logger.info(f"Started FFmpeg streaming for task {task_id}")
            ...
            while stream_info['active']:
                try:
                    ...
                    if frame.shape[:2] != (stream_info['height'], stream_info['width']):
                    ...
                    frame_count += 1
                    if frame_count % 60 == 0:  # Log every 60 frames (roughly every 2 seconds at 30fps)
                        logger.debug(f"Streamed {frame_count} frames for task {task_id}")

                except queue.Empty:
                    continue
                except BrokenPipeError:
                    logger.warning(f"FFmpeg pipe broken for task {task_id}")
                    break
                except Exception as e:
                    logger.error(f"Error streaming frame for task {task_id}: {e}")
                    break

            logger.info(f"Stream worker completed for task {task_id}, processed {frame_count} frames")
    ...
    def _cleanup_stream(self, task_id: str):
        ...
        if task_id in self.streams:
            ...
            if stream_info.get('process'):
                try:
                    stream_info['process'].stdin.close()
                    stream_info['process'].terminate()
                    stream_info['process'].wait(timeout=5)
                    logger.debug(f"FFmpeg process terminated for task {task_id}")
                except subprocess.TimeoutExpired:
                    stream_info['process'].kill()
                    logger.warning(f"FFmpeg process killed for task {task_id}")
                except Exception as e:
                    logger.error(f"Error cleaning up FFmpeg process for task {task_id}: {e}")
        ...
        if task_id in self.output_queues:
            try:
                ...
                while not self.output_queues[task_id].empty():
                    self.output_queues[task_id].get_nowait()
            except:
                pass
    ...
    def is_stream_active(self, task_id: str) -> bool:
        ...
        return task_id in self.streams and self.streams[task_id]['active']
