...
async def root():
    return {
        "service": "RTMPose Service",
        "version": "1.0.0",
        "description": "Microservice for RTMPose pose estimation with dynamic task_id support",
        "endpoints": {
...
async def complete_service(task_id: str, request: TaskCompletionRequest):
    ...
    if not service_info:
        raise HTTPException(status_code=404, detail=f"RTMPose service with task_id '{task_id}' not found")
    ...
    return ServiceResponse(
        success=True,
        message=f"RTMPose service marked as completed for task_id: {task_id}"
    )
...
async def stop_all_services():
    ...
    return {"success": True, "message": "All RTMPose services stopped successfully"} 