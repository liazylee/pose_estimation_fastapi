...
class ServiceManager:
    """Manages multiple RTMPose service instances."""
    ...
    async def start_service(self, task_id: str, config_path: str = "dev_pose_estimation_config.yaml",
        ...
        """Start a RTMPose service with the given task_id."""
        ...
        async with self._lock:
            if task_id in self.services:
                if self.services[task_id]["status"] in ["running", "starting"]:
                    return {
                        "success": False,
                        "message": f"RTMPose service with task_id '{task_id}' is already running"
                    }

            try:
                # Create service instance
                service = RTMPoseService(config_path=config_path, task_id=task_id)

                # Override config with provided parameters
                if devices:
                    service.config.setdefault('rtmpose', {})['devices'] = devices
                if log_level:
                ...
                logger.info(f"Started RTMPose service for task_id: {task_id} (max_restarts: {max_restarts})")

                return {
                    "success": True,
                    "message": f"RTMPose service started successfully for task_id: {task_id}",
                    "topics": {
                        "input_frames": f"raw_frames_{task_id}",
                        "input_detections": f"yolox_detections_{task_id}",
                        "output": f"rtmpose_results_{task_id}",
                        "consumer_groups": [
                            f"rtmpose_raw_{task_id}",
                            f"rtmpose_det_{task_id}"
                        ]
                    }
                }

            except Exception as e:
                error_msg = f"Failed to start RTMPose service for task_id '{task_id}': {str(e)}"
    ...
    async def _run_service(self, task_id: str, service: RTMPoseService):
        ...
        while True:
            try:
                ...
                logger.info(f"RTMPose service for task_id '{task_id}' was cancelled")
                async with self._lock:
                    if task_id in self.services:
                ...
                error_msg = f"RTMPose service error for task_id '{task_id}': {str(e)}"
                logger.error(error_msg)
                ...
                async with self._lock:
                    if task_id in self.services:
                        ...
                        if restart_count >= max_restarts:
                            ...
                            logger.warning(
                                f"Restarting RTMPose service {task_id} (attempt {restart_count}/{max_restarts})")
    ...
    async def stop_service(self, task_id: str) -> Dict[str, Any]:
        ...
        async with self._lock:
            if task_id not in self.services:
                return {
                    "success": False,
                    "message": f"No RTMPose service found with task_id: {task_id}"
                }
        ...
        try:
            ...
            if task_id in self.tasks:
                ...
                try:
                    ...
                    logger.warning(f"Timeout waiting for RTMPose service {task_id} to stop")
            ...
            logger.info(f"Stopped RTMPose service for task_id: {task_id}")

            return {
                "success": True,
                "message": f"RTMPose service stopped successfully for task_id: {task_id}"
            }

        except Exception as e:
            error_msg = f"Failed to stop RTMPose service for task_id '{task_id}': {str(e)}"
            logger.error(error_msg)
            return {
                "success": False,
                "message": error_msg
    ...
    def get_service_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        """Get status of a specific RTMPose service."""
        return self.services.get(task_id)

    def get_all_services(self) -> Dict[str, Dict[str, Any]]:
        """Get status of all RTMPose services."""
        return self.services.copy()

    async def cleanup_all(self):
        """Stop and cleanup all services."""
        logger.info("Stopping all RTMPose services...")
        ...
        logger.info("All RTMPose services stopped")
    ...
    def _start_monitoring(self):
        ...
        logger.info("RTMPose service monitoring started")
